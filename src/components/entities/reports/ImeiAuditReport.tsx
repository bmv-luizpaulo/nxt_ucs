...
import { QrCode, Cpu, Scale, ShieldCheck } from "lucide-react";
              ...
              <table className="w-full text-left">
                <thead className="bg-slate-50">
                  <tr className="border-b-2 border-slate-200">
                    <th className="text-[11px] font-black uppercase text-slate-500 py-4 pl-10">Propriedade / Produtor</th>
                    <th className="text-[11px] font-black uppercase text-slate-500 py-4 text-center">Safra</th>
                    <th className="text-[11px] font-black uppercase text-slate-500 py-4 text-right pr-10">Retido IMEI (UCS)</th>
                  </tr>
                </thead>
                <tbody>
                  {linkedProducers.map((p, idx) => (
                    <tr key={idx} className="border-b border-slate-100 h-16">
                      ...
                      <td className="text-[12px] font-black text-center text-slate-400">{p.safra}</td>
                      <td className="text-[13px] font-black text-right pr-10 text-violet-700 font-mono tracking-tighter">{formatUCS(p.imeiSaldo)}</td>
                    </tr>
                  ))}
                  <tr className="bg-slate-950 text-white h-20">
                    <td colSpan={2} className="text-[13px] font-black uppercase pl-10 tracking-[0.2em]">Total Garantido pelo IMEI</td>
                    <td className="text-[24px] font-black text-right pr-10 text-violet-400 font-mono tracking-tighter">{formatUCS(stats.totalSaldoImei)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div className="report-page px-16 py-16">
          ...
          <div className="grid grid-cols-3 gap-10 mb-20">
            ...
            <div className="bg-slate-950 p-12 rounded-[4rem] border-4 border-slate-800 flex flex-col items-center text-center shadow-2xl scale-105">
              <p className="text-[12px] font-black text-slate-500 uppercase tracking-widest mb-3 text-violet-400/60">Custódia IMEI Total</p>
          ...
          <footer className="mt-40 pt-20 border-t-8 border-slate-950 flex justify-between items-start grayscale transition-all">
            <div className="space-y-8 max-w-xl">
              <div className="flex items-center gap-6 text-slate-950 font-black text-[22px] uppercase tracking-[0.4em]">
                <ShieldCheck className="w-10 h-10 text-emerald-600" /> Conformidade IMEI
              </div>
