...
export default function IMEIPage() {
  ...
  const fetchImeiData = async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/legado/domain?domain=imei');
      if (!res.ok) throw new Error('Erro ao buscar dados do IMEI');
  ...
  // Adapt transactions format to what IMEITransactionTable expects
  const transactions = rawTransactions.map((tx: any) => ({
    ...tx,
    justificativa: tx.descricao,
    origem: tx.tipo === 'entrada' ? `Dist. ${tx.distribution_id}` : 'Carteira IMEI',
    destino: tx.tipo === 'entrada' ? 'Carteira IMEI' : `Dist. ${tx.distribution_id}`,
  }));
          ...
          <div className="flex items-center justify-between relative z-10">
            <div className="flex items-center gap-6">
              ...
              <div className="space-y-1">
                <h1 className="text-[32px] font-black text-white uppercase tracking-tighter leading-none">Controladoria IMEI — Banco Legado</h1>
            ...
            <div className="flex items-center gap-4">
               <Button onClick={() => window.print()} variant="outline" className="h-11 px-6 rounded-2xl border-white/10 text-white text-[10px] font-black uppercase tracking-widest gap-2 bg-white/5 hover:bg-white/10 transition-all shadow-xl">
                  <Download className="w-4 h-4 text-indigo-400" /> Exportar Auditoria (JSON/XLSX)
               </Button>
               <Button onClick={fetchImeiData} className="h-11 px-8 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-widest shadow-2xl transition-all active:scale-95">
