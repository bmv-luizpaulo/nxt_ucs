...
import { Trash2, ChevronLeft, ChevronRight, Loader2, Users2 } from "lucide-react";
...
function NucleosContent() {
  ...
  const statusFiltered = useMemo(() => {
    return (allProdutores || [])
      ...
      .map(item => ({ ...item, volumeContextual: item.associacaoSaldo || 0 }));
  }, [allProdutores, activeTab]);

  const filteredData = useEntityFilters(statusFiltered, filters, 'nucleo');
          ...
          {isLoading ? <div className="flex items-center justify-center h-64"><Loader2 className="w-8 h-8 text-primary animate-spin" /></div> : (
            <div className="space-y-6">
              ...
              <EntityFilters
                ...
                onChange={setFilters}
                config={{
                  searchPlaceholder: "Buscar núcleo, associação ou produtor...",
              ...
              <EntityTable data={paginated} selectedIds={selectedIds} onSelectionChange={setSelectedIds} onUpdate={handleUpdate} viewMode="nucleo" />
