...
function NucleosContent() {
  ...
  // 1. Consulta de Produtores (para Saldos)
  const allProdutoresQuery = useMemoFirebase(() => {
    if (!firestore || !user) return null;
    return query(collection(firestore, "produtores"), orderBy("nome", "asc"));
  }, [firestore, user]);
  const { data: allProdutores, isLoading: isProdutoresLoading } = useCollection<EntidadeSaldo>(allProdutoresQuery);

  // 2. Consulta de Fazendas (para Contagem de Propriedades)
  const allFazendasQuery = useMemoFirebase(() => {
    if (!firestore || !user) return null;
    return query(collection(firestore, "fazendas"));
  }, [firestore, user]);
  const { data: allFazendas, isLoading: isFazendasLoading } = useCollection<any>(allFazendasQuery);

  const isLoading = isProdutoresLoading || isFazendasLoading;
  ...
  const nucleoList = useMemo(() => {
    const map = new Map<string, { count: number; totalSaldo: number; totalOrig: number; isAssoc?: boolean; id?: string; cnpj?: string }>();

    // Primeiro, pegar as Associações cadastradas na coleção produtores
    (allProdutores || []).filter(p => (p as any).isAssociacao).forEach(p => {
      const key = p.nucleo || p.nome;
      if (!map.has(key)) {
         map.set(key, { 
           count: 0, 
           totalSaldo: (p as any).associacaoSaldo || 0, 
           totalOrig: (p as any).originacao || 0,
           isAssoc: true,
           id: p.id,
           cnpj: p.documento
         });
    ...
    // Depois, somar dados das Fazendas Reais
    (allFazendas || []).forEach(f => {
      const key = f.nucleo || "Sem Núcleo";
      if (!map.has(key)) map.set(key, { count: 0, totalSaldo: 0, totalOrig: 0 });
      const entry = map.get(key)!;
      entry.count++;
    });

    // Saldo baseado nos Produtores (Filtrado por Status)
    statusFiltered.forEach(p => {
      const key = p.nucleo || p.associacaoNome || "Sem Núcleo";
      if (!map.has(key)) return;
      const entry = map.get(key)!;
      // Não somar se o próprio produtor for a associação (evitar duplicar saldo na visão do núcleo)
      if (!(p as any).isAssociacao) {
  ...
  }, [allFazendas, allProdutores, statusFiltered]);
              ...
              <div className="flex gap-3 pb-4">
                {nucleoList.map(nucleo => (
                  <div
                    ...
                    className={cn(
                      "shrink-0 px-6 py-4 rounded-[1.5rem] border-2 transition-all text-left min-w-[200px] cursor-pointer",
                  ...
                  >
                    ...
                    <div className="flex items-end justify-between">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-slate-400 uppercase">{nucleo.count} PROPRIEDADES</span>
                        <button
                          onClick={(e) => { e.stopPropagation(); router.push(`/nucleos/${encodeURIComponent(nucleo.name)}`); }}
                          className="text-[8px] font-black text-emerald-600 uppercase tracking-widest mt-2 hover:underline"
                        >
                          Acessar Dashboard →
                        </button>
                      </div>
