...
async function run() {
  ...
  const csvPath = path.join(__dirname, '../../docs/banco_legado_csvs_completo/dbo_account_adjustments_order.csv');
  const total = await processFile(csvPath, 'accountAdjustments', 'Ajustes de Contas (accountAdjustments)');

  console.log(`\n✅ Sucesso! Total accountAdjustments migrado: ${total} registros`);
  process.exit(0);
}
