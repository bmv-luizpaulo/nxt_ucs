...
async function run() {
  ...
  const csvPath = path.join(__dirname, '../../docs/banco_legado_csvs_completo/dbo_role_user.csv');
  const total = await processFile(csvPath, 'roleUsers', 'Perfis de Usuários (roleUsers)');

  console.log(`\n✅ Sucesso! Total roleUsers migrado: ${total} registros`);
  process.exit(0);
}
