  ...
  return new Promise((resolve, reject) => {
    ...
    parser.on('data', async (row) => {
      ...
      const data: Record<string, unknown> = {
        ...
        block: row.block === 't' || row.block === 'true',
...
async function run() {
  ...
  const csvPath = path.join(__dirname, '../../docs/banco_legado_csvs_completo/dbo_blocked_ucs.csv');
  const total = await processFile(csvPath, 'blockedUcs', 'UCS Bloqueadas (blockedUcs)');

  console.log(`\n✅ Sucesso! Total blockedUcs migrado: ${total} registros`);
  process.exit(0);
}
